...
function Socket(options) {
  ...
  this.wrap = new raw.SocketWrap(
    options && options.protocol ? options.protocol : 0,
    options && options.addressFamily
      ? options.addressFamily
      : AddressFamily.IPv4,
  );
  if (options && options.bindToDevice && this.bindToDevice) {
      this.bindToDevice(options.bindToDevice);
  }

  if (options && options.bindToAddress) {
      this.bindToAddress(options.bindToAddress);
  }
...
Socket.prototype.setOption = function (level, option, value, length) {
  if (arguments.length > 3) this.wrap.setOption(level, option, value, length);
  else this.wrap.setOption(level, option, value);
};
